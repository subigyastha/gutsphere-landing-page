...
const guideIds = ['stool-color', 'gut-inflamed', 'mucus-stool'] as const
const videoId = 'rectal-signal'

export function YouV2ContentLibrary() {
  ...
  return (
    <section id="content" className="section-pad bg-gs-sand-light" aria-labelledby="you-v2-content-heading">
      <div className="container-narrow">
        <p className="text-xs font-medium text-gs-coral">Free guides &amp; videos</p>
        <h2 id="you-v2-content-heading" className="section-heading mt-3 max-w-2xl">
          Learn first — track what matches your body
        </h2>
        <p className="body-lg mt-3 max-w-2xl sm:mt-4">
          Prepping for a visit or still piecing symptoms together? Start with trusted guides and
          short videos — then log what you notice in Gutsphere.
        </p>
        ...
        <div className="mt-8 sm:mt-10">
          ...
          >
            ...
            <div className="flex flex-col justify-center p-5 sm:p-6 lg:w-2/3 lg:py-8 lg:pr-8">
              <p className="text-sm font-medium text-gs-coral">Before your next visit</p>
        ...
        <div className="mt-6 grid gap-3 sm:mt-8 sm:grid-cols-2 lg:grid-cols-4 sm:gap-4">
          ...
          {video && (
            ...
            >
              ...
              {!video.thumbnail && (
                <p className="text-xs font-medium text-gs-text-muted">Video &middot; {video.duration}</p>
              )}
              <h3 className="mt-1 font-display text-base font-semibold text-gs-text-primary group-hover:text-gs-coral sm:text-lg">
                {video.title}
              </h3>
              <span className="mt-auto inline-flex min-h-12 items-center pt-3 text-sm font-semibold text-gs-coral">
                Watch video &rarr;
